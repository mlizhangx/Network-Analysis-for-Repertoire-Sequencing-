...
# build network, find public clusters (one sample)
    ...
    output_type_unfiltered, ...)
{
  ...
  ### FIND PUBLIC CLUSTERS IN SAMPLE ###
  cat(">>> Filtering clusters in the current sample...")
  ndat <- net$node_data; cdat <- net$cluster_data
  if (is.null(count_col)) { min_clone_count <- NULL }
  cdat <- .filterClustersBySize(cdat,
                                top_n_clusters, min_node_count, min_clone_count)

  # Subset node data keeping only the filtered clusters
  ndat <- ndat[ndat$cluster_id %in% cdat$cluster_id , ]
  cat(paste0(" Done.\n* ", nrow(cdat), " clusters (", nrow(ndat), " nodes) remain. "))

  ### FORMAT AND SAVE DATA FOR SAMPLE ###
  # Rename columns of node and cluster stats to distinguish sample-level stats
...
# filter clusters by node/clone count; return row ids of filtered clusters
...
# rename columns of node data to distinguish sample-level network properties
    ...
    ndat, cdat, output_type, output_dir, sample_id)
{
  ...
  .createOutputDir(node_dir); .createOutputDir(cluster_dir)
...
.addClusterOnClusterStats <- function(ndat, cdat, adjacency_matrix) {
  ...
  for (i in 1:nrow(cdat)) {
    ...
    cluster_id <- which(cdat$ClusterIDPublic == i) # current row of cluster ndat
    node_ids <- ndat$ClusterIDPublic == i  # Rows of node ndat for current cluster
    ...
    cdat$MeanOfMeanSeqLength[[cluster_id]] <-
      round(mean(ndat[node_ids, "mean_seq_length"]), 2)

    # Mean degree in cluster
    cdat$MeanDegreeInPublicNet[[cluster_id]] <-
      round(mean(ndat[node_ids, "degree"]), 2)
    ...
    cdat$DiameterLength[[cluster_id]] <-
...
#       stopifnot(
#         "must specify data symbol via argument `data_symbols`" =
#           !is.null(data_symbols)
#       )
#       load(file.path(input_dir, file_list[[i]]))
#       assign(x = "data", value = get(data_symbols))
#     } else {
