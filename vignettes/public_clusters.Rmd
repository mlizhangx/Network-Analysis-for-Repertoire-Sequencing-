...
For details on how the pattern matching is performed, refer to the base R documentation files for `regex()` and `grep()`.
...
This is accomplished using `addClusterLabels()` [as described here](network_visualization.html#labeling_clusters). Note that within the node-level metadata, the global cluster ID is stored in the variable `ClusterIDPublic`, so we must provide this column name to the `cluster_id_col` argument. 
...
unlink(list.dirs(tempdir(), full.names = TRUE),
       recursive = TRUE)
```
