...
The main argument of the `findPublicClusters` function is the `file_list` argument, which accepts a vector containing file paths. Each file is assumed to contain the AIRR-Seq data for a single sample, with observations indexed by row.
...
By default, clustering within each sample's network is performed using the `cluster_fast_greedy` algorithm from the `igraph` package. A different clustering algorithm can be specified using the `cluster_fun` argument. See [this section of the buildRepSeqNetwork vignette](buildRepSeqNetwork.html#cluster_alg) for details.



## Output Settings for Step 1 {.tabset}

The `findPublicClusters` function does not return any direct output. Instead, it saves data for the selected clusters to files that will be used as inputs in step 2. 
...
The full network data for each sample includes [all of the objects contained in the output of `buildRepSeqNetwork`](buildRepSeqNetwork.html#base_output), including cluster metadata. By default, these are placed in a list named `net` and saved to an RDS file whose file name is the [sample ID](#sample_ids). The RDA format can be used instead of RDS by setting `output_type_unfiltered = "rda"`. Alternatively, setting `output_type_unfiltered = "individual"` saves each R object using the [default file formats from `buildRepSeqNetwork`](buildRepSeqNetwork.html#output_format), where each file's name begins with the [sample ID](#sample_ids).
...
By default, the plot for each sample will color the nodes according to the cluster to which they belong. The user can specify a different variable for coloring the nodes using the `color_nodes_by` argument  [as detailed here](network_visualization.html#node_colors). It is even possible to generate [multiple plots of each sample's network, with each plot colored according to a different variable](network_visualization.html#multiple_plots).
...
We execute the `findPublicClusters` function using the [input we prepared earlier](#files1) for the `file_list` argument.
...
Refer [here](buildRepSeqNetwork.html#node_properties) for more information about the node-level network properties.
...
In [step 1](#step1), we used the `findPublicClusters` function to perform network analysis on each sample individually and select clusters based on node count and clone count.

Next, we will use the `buildPublicClusterNetwork` function to combine the filtered data from all samples into a single global network and perform clustering analysis.  
...
By default, clustering within the global network is performed using the `cluster_fast_greedy` algorithm from the `igraph` package. A different clustering algorithm can be specified using the `cluster_fun` argument. See [this section of the buildRepSeqNetwork vignette](buildRepSeqNetwork.html#cluster_alg) for details.
...
The output returned by `buildPublicClusterNetwork` follows the [same format as the output of `buildRepSeqNetwork`](buildRepSeqNetwork.html#output_settings). The function returns a list containing the [node-level](#node_variables) and [cluster-level](#cluster_variables) metadata for the global network, as well as any plots generated, in addition to the network adjacency matrix and the `igraph` network edge list.
...
We execute the `buildPublicClusterNetwork` function using the [input we prepared earlier](#files2) for the `file_list` argument:
...
This is accomplished using the `addClusterLabels` function [as described here](network_visualization.html#labeling_clusters). Note that within the node-level metadata, the global cluster ID is stored in the variable `ClusterIDPublic`, so we must provide this column name to the `cluster_id_col` argument. 
