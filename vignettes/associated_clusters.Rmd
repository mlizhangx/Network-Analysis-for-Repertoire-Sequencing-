...
By default, `findAssociatedClones` saves its output files using the csv file format. By default,  `buildAssociatedClusterNetwork` assumes that the input files are in the csv file format. 

If the files created by `findAssociatedClones` were saved using a different file format, the `input_type` argument can be used to specify the format used. This argument has the [same behavior](#input_files) as seen in `findAssociatedSeqs` and `findAssociatedClones`.
