...
The main argument of the `findAssociatedSeqs` function is the `file_list` argument, which accepts a vector containing file paths. Each file is assumed to contain the AIRR-Seq data for a single sample, with observations indexed by row.
...
We execute the `findAssociatedSeqs` function using the [inputs we prepared earlier](#input_settings) for the [`file_list`](#input_files) and [`group_ids`](#group_ids) arguments:
...
We execute the `findAssociatedClones` function using the [inputs from step 1](#files2) for the [`file_list`](#input_files) and [`group_ids`](#group_ids) arguments, the [associated sequences from step 1](#assoc_seqs), as well as the [output directory file path we created for the current step](#output2):
...
We execute the `buildAssociatedClusterNetwork` function using the [input we prepared earlier](#input_files3) for the `file_list` argument:
...
As an additional step in our analysis, we can take the TCR sequences from the global network and use a deep learning algorithm to encode each TCR sequence as a 30-dimensional numeric vector based on the Atchley-factor representations of its amino acids. This is done using the software [TESSA](https://doi.org/10.1038/s41592-020-01020-3).
...
All of these steps are performed by the function `kmeansAtchley()`.
