...
For example, we can color the nodes based on the variable `CloneCount`:
```{r}
...
If `buildRepSeqNetwork` is called with `cluster_stats = TRUE`, the results can be used to label particular clusters with the `addClusterLabels` function:
...
The `addClusterLabels` function has the following arguments:

* `plot` is a ggraph object such as those contained in the output returned by `buildRepSeqNetwork`
* `net` is a corresponding list of network objects, such as that returned by `buildRepSeqNetwork`, that includes the list items `node_data` and `cluster_data`
* `top_n_clusters` specifies how many clusters to label (based on `criterion`).
* `criterion` is the name of a numeric cluster statistic (see `?getClusterStats`) for ranking the clusters. The default is node count.
* `size` and `color` control the size and color of the labels
