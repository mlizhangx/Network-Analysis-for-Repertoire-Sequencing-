...
The nodes in the graph can be colored according to a variable in the node-level metadata. This is accomplished using the `color_nodes_by` argument, which accepts a character string specifying the name of the column containing the variable.

The user can specify any variable that will be present in the node-level meta data returned by `buildRepSeqNetwork`, including node-level network properties. By default, this also includes all variables that were present in the original input data. 
...
This section covers functionality within the `NAIR` package that facilitates additional visual analysis of the results from the `buildRepSeqNetwork` function.
