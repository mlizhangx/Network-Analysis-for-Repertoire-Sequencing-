...
`plots` is a list containing any plots created as well as a matrix `graph_layout` storing the node layout used in the plots. Each plot is named according to the variable used to color the nodes. The default (uncolored) plot is named `uniform_color`. 
